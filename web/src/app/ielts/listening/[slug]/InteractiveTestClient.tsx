...
export default function InteractiveTestClient({ initialTest }: { initialTest: any }) {
    ...
    const options = {
      replace: (domNode: any) => {
        ...
        if (domNode.name === 'input') {
          ...
          if (!valueAttr && domNode.next && domNode.next.type === 'text') {
            valueAttr = domNode.next.data.trim();
          }
          
          if (!valueAttr && domNode.next && domNode.next.name === 'strong' && domNode.next.children?.[0]?.type === 'text') {
            valueAttr = domNode.next.children[0].data.trim();
          }
          ...
          if (typeAttr === 'radio' || typeAttr === 'checkbox') {
             return (
               <input
                 ...
                 name={`q-${qNum}`}
                 value={valueAttr}
                 checked={typeAttr === 'radio' ? answers[qNum] === valueAttr : (answers[qNum]?.includes(valueAttr) || false)}
                 onChange={(e) => {
                   if (typeAttr === 'radio') {
                  ...
                  {q.questionType === 'Multiple Choice' ? (
                    <div className="options-list">
                      {q.options?.split(',').map((opt: string) => {
                        ...
                        return (
                          <label key={optionText} className="option-label" style={{ alignItems: 'flex-start', width: '100%' }}>
                            <input 
                              type="radio" 
                            ...
                            />
                            <span style={{ flex: 1, wordBreak: 'break-word', lineHeight: 1.5 }}>{optionText}</span>
                          </label>
                        )
                      })}
                    </div>
